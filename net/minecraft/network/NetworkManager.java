...
public class NetworkManager extends SimpleChannelInboundHandler < Packet<? >>
{
    ...
    {
        //MEOWWARE HOOK
        PacketSendEvent packetEvent = new PacketSendEvent(p_channelRead0_2_);
        packetEvent.call();
    ...
    {
        //MEOWWARE HOOK
        PacketReceiveEvent packetEvent = new PacketReceiveEvent(inPacket);
        packetEvent.call();
    ...
    public static NetworkManager createNetworkManagerAndConnect(InetAddress address, int serverPort, boolean useNativeTransport)
    {
        ...
        (new Bootstrap()).group(lazyloadbase.getValue()).handler(new ChannelInitializer<Channel>()
        {
            protected void initChannel(Channel p_initChannel_1_) {
                ...
                if (MeowWare.getInstance().getProxyType() != ProxyType.NONE) {
                    try {
                        final String s = MeowWare.getProxyIP();
                        final String[] astring = s.split(":");
                        final int i = Integer.parseInt(astring[1]);
                        final InetSocketAddress inetsocketaddress = new InetSocketAddress(astring[0], i);
                        if (MeowWare.getInstance().getProxyType() == ProxyType.SOCKS4) {
                            final Socks4ProxyHandler socks4proxyhandler = new Socks4ProxyHandler(inetsocketaddress);
                            p_initChannel_1_.pipeline().addFirst(socks4proxyhandler);
                        }
                        else if (MeowWare.getInstance().getProxyType() == ProxyType.SOCKS5) {
                            final Socks5ProxyHandler socks5proxyhandler = new Socks5ProxyHandler(inetsocketaddress);
                            p_initChannel_1_.pipeline().addFirst(socks5proxyhandler);
                        }
                        else if (MeowWare.getInstance().getProxyType() == ProxyType.HTTP) {
                            final HttpProxyHandler httpProxyHandler = new HttpProxyHandler(inetsocketaddress);
                            p_initChannel_1_.pipeline().addFirst(httpProxyHandler);
                        }
                    }
