...
import wtf.whess.meowware.client.main.functions.impl.visual.SwingAnimation;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        if (world == null)
            return 0;
        float i = MeowWare.getInstance().getFunctionManager().getFunctionClass(SwingAnimation.class).isToggled() ? (float) (1F / SwingAnimation.getSpeed().getValue()) : 1;

        if (this.isPotionActive(MobEffects.HASTE))
        {
            return (int) ((6 - (1 + this.getActivePotionEffect(MobEffects.HASTE).getAmplifier())) * i);
        }
        else
        {
            return (int) ((this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : 6) * i);
        }
    }
    ...
    /**
     * Sends an ENTER_COMBAT packet to the main
     */
    ...
    /**
     * Sends an END_COMBAT packet to the main
     */
